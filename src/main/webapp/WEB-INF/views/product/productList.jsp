	...
	<form action="./productList">
	    ...
	    <tbody>
	      <tr>
			...
			<td>
			<c:if test="${vo.sell eq '1'}">
			<div><button class="btn-subway-gray Soldout" type="button" data-toggle="modal" data-target="#myModal" data-nonotifyid="${vo.sell}" data-notifyid="${vo.menuNum}" >품절</button></div>
			</c:if>
			
			<c:if test="${vo.sell eq '0'}">
			<div><button class="btn-subway-gray Soldout" type="button" data-toggle="modal" data-target="#myModal" data-nonotifyid="${vo.sell}" data-notifyid="${vo.menuNum}" >품절 해제</button></div>
			</c:if>
			</td>
  ...
  <div class="modal-dialog modal-center" role="document">
    <div class="modal-content">
       ...
       <div class="modal-header">
		<div class="subway_logo1"style="text-align: center; margin-left: 30%;" href="./productList" >
...
<!-- Pager -->
	...
	<div style="margin: 0 auto;">
	 ...
	 <c:if test="${pager.curPage>1}">
		<li class="page-item" ><a class="page-link" href="./productList?curPage=${pager.curPage-1}&kind=${pager.kind}&search=${pager.search}">이전</a></li>
	</c:if>
	
	<c:forEach begin="${pager.startNum}" end="${pager.lastNum}" var="i">
		 <li class="page-item"><a class="page-link" href="./productList?curPage=${i}&kind=${pager.kind}&search=${pager.search}">${i}</a></li>
	 </c:forEach>

	<c:if test="${pager.curPage<pager.totalPage}">
	  	<li class="page-item"><a class="page-link" href="./productList?curPage=${pager.curPage+1}&kind=${pager.kind}&search=${pager.search}">다음</a></li>
	</c:if>
	 </ul>
	</div>
</c:if>
