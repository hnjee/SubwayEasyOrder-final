 ...
 <div id="wrapper">
	...
	<form action="./headOfficeIngredientList">
		<div class="serach">
	        ...
	        <input type="button" class=del-btn-all value="선택삭제">
	...
	<!--   <span><button class=del-btn><i class='fas fa-times'></i> 삭제하기</button></span>  -->
	  ...
	  <table>
	    ...
	    <tbody>
	      <tr>
	        <td><input class="rowCheck checks" type="checkbox" name="${vo.ingreNum}" ></td>
	        <td>${vo.ingreNum}</td>
	        <td>${vo.ingreCode}</td>
	        <td>${vo.name}</td>
	        <td>${vo.price}원</td>
			...
			<td>
			<c:if test="${vo.sale==0}">
			<div><button class="btn-subway-gray " title="${vo.ingreNum}" type="button" data-toggle="modal" data-target="#myModal"data-notifyid="${vo.ingreNum}" data-nonotifyid="${vo.sale}">품절해제</button></div>
			</c:if>
			<c:if test="${vo.sale==1}">			
			<div><button class="btn-subway-gray" title="${vo.ingreNum}" type="button" data-toggle="modal" data-target="#myModal"data-notifyid="${vo.ingreNum}" data-nonotifyid="${vo.sale}">품절</button></div>
			</c:if>	
			<div><a href="./ingredientUpdate?ingreNum=${vo.ingreNum}"><button class="btn-subway-gray" type="button"> 수정</button></a></div>
			<div><a href="./ingredientDelete?ingreNum=${vo.ingreNum}"><button class="btn-subway-gray deleteOne" type="button"> 삭제</button></a></div>
...
<c:if test="${pager.totalPage>0}">
 ...
 <!-- Paging end -->  
  ...
  <a class="scroll-to-top rounded" href="#page-top">
	...
	<script src="../js/ingredient/headOfficeIngredientList.js" type="text/javascript"></script>
</html>